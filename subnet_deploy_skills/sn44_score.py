...
Usage:
  python sn44_score.py deploy --wallet finflow_miner --machine beast
  python sn44_score.py status
  python sn44_score.py stop
...
SUBNET = {
    "netuid": 44,
    "name": "SCORE Vision",
    "repo": "https://github.com/score-technologies/score-vision.git",
    "repo_dir": "score-vision",
    "github": "score-technologies/score-vision",
    "min_vram_gb": 8,
    "python_version": "3.10",
    "difficulty": "Medium",
    "type": "Computer Vision YOLO",
...
def check_gpu():
    ...
    if not result:
        print("  No NVIDIA GPU found. SN44 needs GPU with 8GB+ VRAM.")
        return False
...
def setup_venv(repo_path):
    """Set up Python virtual environment with Fiber framework."""
    print("\n[3/7] Setting up Python environment (Fiber framework)...")
    ...
    print("  Installing fiber[full]...")
    run(f"{pip} install 'fiber[full]' --quiet", cwd=repo_path, timeout=300, check=False)
    ...
    print("  Dependencies installed (Fiber + project deps)")
    return True
...
def register(wallet_name, hotkey_name):
    """Register on SN44. Requires wallet password."""
...
def configure(repo_path, wallet_name, hotkey_name):
    ...
    config = f"""# SN44 SCORE Vision — Auto-configured by SubnetAIQ Deploy Skill
...
def start_miner(repo_path, wallet_name, hotkey_name):
    ...
    miner_name = f"sn{SUBNET['netuid']}-score"
...
def deploy(wallet_name, hotkey_name):
    ...
    print(f"  NOTE: Uses Fiber framework (not standard Bittensor)")
    print(f"{'='*60}")
    ...
    print(f"  Miner: sn{SUBNET['netuid']}-score")
...
if __name__ == "__main__":
    ...
    if args.action == "deploy":
        ...
        run(f"pm2 stop sn{SUBNET['netuid']}-score", check=False)
        print("Miner stopped.")
