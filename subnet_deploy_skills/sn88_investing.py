"""SubnetAIQ Deploy Skill — SN88 Investing
...
5. Configures environment + creates strategy file
6. Starts miner via PM2
7. Verifies earning status

Usage:
  python sn88_investing.py deploy --wallet finflow_miner --machine beast
  python sn88_investing.py status
  python sn88_investing.py stop
...
Requirements:
  ...
  - PM2 installed
  - Strategy file in Investing/strat/{hotkey_ss58}
"""
...
SUBNET = {
    "netuid": 88,
    "name": "Investing",
    "repo": "https://github.com/mobiusfund/investing.git",
    "repo_dir": "investing",
    "github": "mobiusfund/investing",
    "min_vram_gb": 0,
    "python_version": "3.10",
    "difficulty": "Easy",
    "type": "Investment Strategy Filing",
...
def check_gpu():
    """No GPU required for SN88 — CPU only subnet."""
    print("\n[1/7] Checking hardware...")
    print("  SN88 Investing is CPU-only. No GPU required.")
...
def register(wallet_name, hotkey_name):
    """Register on SN88. Requires wallet password."""
...
def configure(repo_path, wallet_name, hotkey_name):
    """Configure the miner environment and create strategy file."""
    print("\n[6/7] Configuring miner...")

    # Write env config
    env_file = os.path.join(repo_path, "miner.env")
    config = f"""# SN88 Investing — Auto-configured by SubnetAIQ Deploy Skill
...
"""
    ...
    # Create strategy directory and placeholder file
    hotkey_path = os.path.expanduser(f"~/.bittensor/wallets/{wallet_name}/hotkeys/{hotkey_name}")
    if os.path.exists(hotkey_path):
        with open(hotkey_path, "r") as f:
            hotkey_data = json.load(f)
        ss58 = hotkey_data.get("ss58Address", "UNKNOWN")

        strat_dir = os.path.join(repo_path, "Investing", "strat", ss58)
        os.makedirs(strat_dir, exist_ok=True)
        print(f"  Strategy dir created: {strat_dir}")
        print(f"  WARNING: You must place your strategy file in: {strat_dir}")
        print(f"  See the Investing README for strategy file format.")
    else:
        print(f"  WARNING: Hotkey not found — create strategy dir manually after hotkey creation")
        print(f"  Strategy dir pattern: {repo_path}/Investing/strat/{{hotkey_ss58}}/")
...
def start_miner(repo_path, wallet_name, hotkey_name):
    ...
    miner_name = f"sn{SUBNET['netuid']}-investing"
...
def deploy(wallet_name, hotkey_name):
    ...
    print(f"  NOTE: Requires strategy file in Investing/strat/{{ss58}}/")
    print(f"{'='*60}")
    ...
    print(f"  Miner: sn{SUBNET['netuid']}-investing")
...
if __name__ == "__main__":
    ...
    if args.action == "deploy":
        ...
        run(f"pm2 stop sn{SUBNET['netuid']}-investing", check=False)
        print("Miner stopped.")
