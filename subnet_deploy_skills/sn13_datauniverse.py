"""SubnetAIQ Deploy Skill — SN13 Data Universe
...
Usage:
  python sn13_datauniverse.py deploy --wallet finflow_miner --machine beast
  python sn13_datauniverse.py status
  python sn13_datauniverse.py stop
...
SUBNET = {
    "netuid": 13,
    "name": "Data Universe",
    "repo": "https://github.com/macrocosm-os/data-universe.git",
...
def check_gpu():
    """No GPU required for SN13 — CPU only subnet."""
    print("\n[1/7] Checking hardware...")
    print("  SN13 Data Universe is CPU-only. No GPU required.")
...
def register(wallet_name, hotkey_name):
    """Register on SN13. Requires wallet password."""
...
def configure(repo_path, wallet_name, hotkey_name):
    ...
    config = f"""# SN13 Data Universe — Auto-configured by SubnetAIQ Deploy Skill
...
def start_miner(repo_path, wallet_name, hotkey_name):
    ...
    miner_name = f"sn{SUBNET['netuid']}-datauniverse"
...
def deploy(wallet_name, hotkey_name):
    ...
    print(f"  NOTE: CPU-only subnet. No GPU required.")
    print(f"{'='*60}")
    ...
    print(f"  Miner: sn{SUBNET['netuid']}-datauniverse")
...
if __name__ == "__main__":
    ...
    if args.action == "deploy":
        ...
        run(f"pm2 stop sn{SUBNET['netuid']}-datauniverse", check=False)
        print("Miner stopped.")
