"""SubnetAIQ Deploy Skill — SN72 StreetVision
...
3. Creates hotkey
4. Registers on-chain (burns ~0.07 TAO)
5. Configures environment
6. Starts miner via PM2
7. Verifies earning status

Usage:
  python sn72_streetvision.py deploy --wallet finflow_miner --machine beast
...
SUBNET = {
    "netuid": 72,
    "name": "StreetVision",
    "repo": "https://github.com/natixnetwork/natix-subnet.git",
    "repo_dir": "subnet-72",
    "github": "natixnetwork/natix-subnet",
    "min_vram_gb": 4,
    "python_version": "3.10",
    "difficulty": "Medium",
    "type": "Image Classification — ViT roadwork detection",
    "burn_estimate_tao": 0.07,
    "axon_port": 9072,
    "model_url": "https://huggingface.co/WinstonFrank/roadwork",
}
...
def check_gpu():
    ...
    if not result:
        print("  No NVIDIA GPU found. SN72 needs GPU with 4GB+ VRAM.")
        return False
...
def setup_venv(repo_path):
    ...
    if os.path.exists(pyproject):
        # Adjust python version constraint if needed
        run(f"sed -i.bak 's/python = \">=3.10,<3.12\"/python = \">=3.10,<3.13\"/' {pyproject}", check=False)
    ...
    # Verify imports
    test = run(f"{python} -c \"import natix; print('OK')\"", cwd=repo_path, check=False)
...
def register(wallet_name, hotkey_name):
    """Register on SN72. Requires wallet password."""
...
def configure(repo_path, wallet_name, hotkey_name):
    ...
    config = f"""# SN72 StreetVision — Auto-configured by SubnetAIQ Deploy Skill
...
MINER_AXON_PORT={SUBNET['axon_port']}
BLACKLIST_FORCE_VALIDATOR_PERMIT=True
MODEL_URL={SUBNET['model_url']}
...
def start_miner(repo_path, wallet_name, hotkey_name):
    ...
    miner_name = f"sn{SUBNET['netuid']}-natix"
...
def deploy(wallet_name, hotkey_name):
    ...
    print(f"  Miner: sn{SUBNET['netuid']}-natix")
...
if __name__ == "__main__":
    ...
    if args.action == "deploy":
        ...
        run(f"pm2 stop sn{SUBNET['netuid']}-natix", check=False)
        print("Miner stopped.")
