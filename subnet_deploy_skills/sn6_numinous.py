"""SubnetAIQ Deploy Skill — SN6 Numinous
Executable mining skill. Agent runs this → agent uploaded → earning TAO.

SPECIAL: SN6 does NOT use a traditional miner process. Instead, miners
upload a forecasting agent via the `numi` CLI tool. There is no long-running
PM2 process — the agent runs on Numinous infrastructure.

What it does:
1. Checks prerequisites (no GPU needed)
2. Installs numinous CLI
3. Creates hotkey
4. Registers on-chain
5. Prints upload instructions
6. N/A — no PM2 process
7. Verifies on-chain status

Usage:
  python sn6_numinous.py deploy --wallet finflow_miner --machine beast
  python sn6_numinous.py status
  python sn6_numinous.py stop

Requirements:
  - Python 3.10+
  - Bittensor wallet with TAO for registration
  - numi CLI (pip install numinous)
"""

import argparse
import json
import os
import subprocess
import sys
import time


# ─── CONFIG ──────────────────────────────────────────────────────────────

SUBNET = {
    "netuid": 6,
    "name": "Numinous",
    "repo": None,
    "repo_dir": "numinous",
    "github": None,
    "min_vram_gb": 0,
    "python_version": "3.10",
    "difficulty": "Medium",
    "type": "Forecasting Agent Upload",
    "burn_estimate_tao": 0.07,
    "axon_port": None,
}


# ─── HELPERS ─────────────────────────────────────────────────────────────

def run(cmd, cwd=None, timeout=120, check=True):
    """Run a shell command and return output."""
    print(f"  $ {cmd}")
    try:
        r = subprocess.run(
            cmd, shell=True, cwd=cwd, capture_output=True, text=True, timeout=timeout
        )
        if check and r.returncode != 0:
            print(f"  ERROR: {r.stderr[:200]}")
            return None
        return r.stdout.strip()
    except subprocess.TimeoutExpired:
        print(f"  TIMEOUT after {timeout}s")
        return None


def check_gpu():
    """No GPU required — agents run on Numinous infrastructure."""
    print("\n[1/7] Checking hardware...")
    print("  SN6 Numinous: No local GPU needed.")
    print("  Agents are uploaded and run on Numinous infrastructure.")
    print("  PASS: No hardware requirements")
    return True


def clone_repo(home_dir):
    """Install numinous CLI instead of cloning a repo."""
    print("\n[2/7] Installing numinous CLI...")
    work_dir = os.path.join(home_dir, SUBNET["repo_dir"])
    os.makedirs(work_dir, exist_ok=True)

    # Install numinous package
    result = run("pip install numinous --quiet", timeout=120, check=False)
    if result is not None:
        print("  numinous package installed")

    # Verify numi CLI
    result = run("which numi", check=False)
    if result:
        print(f"  numi CLI found: {result}")
    else:
        result = run("python3 -m pip show numinous", check=False)
        if result:
            print("  numinous installed but numi not in PATH")
            print("  Try: python3 -m numinous --help")
        else:
            print("  WARNING: numinous not installed. Run: pip install numinous")

    return work_dir


def setup_venv(repo_path):
    """No venv needed — numinous is installed globally."""
    print("\n[3/7] Verifying numinous installation...")
    result = run("python3 -c \"import numinous; print('OK')\"", check=False)
    if result and "OK" in result:
        print("  numinous package verified")
        return True
    print("  WARNING: numinous import failed. Run: pip install numinous")
    return True  # Continue anyway — user may install later


def create_hotkey(wallet_name, hotkey_name):
    """Create a new hotkey if it doesn't exist."""
    print("\n[4/7] Setting up wallet...")
    hotkey_path = os.path.expanduser(f"~/.bittensor/wallets/{wallet_name}/hotkeys/{hotkey_name}")
    if os.path.exists(hotkey_path):
        print(f"  Hotkey {hotkey_name} already exists")
        result = run(f"python3 -c \"import json; print(json.load(open('{hotkey_path}'))['ss58Address'])\"", check=False)
        if result:
            print(f"  Address: {result}")
        return True

    btcli = None
    for path in [
        os.path.expanduser("~/synth-subnet/.venv/bin/btcli"),
        os.path.expanduser("~/subnet-72/.venv/bin/btcli"),
    ]:
        if os.path.exists(path):
            btcli = path
            break

    if not btcli:
        print("  FAIL: btcli not found. Install bittensor first.")
        return False

    result = run(f"echo | {btcli} wallet new_hotkey --wallet.name {wallet_name} --wallet.hotkey {hotkey_name}", check=False)
    if os.path.exists(hotkey_path):
        print(f"  Created hotkey: {hotkey_name}")
        return True
    print("  FAIL: Could not create hotkey")
    return False


def register(wallet_name, hotkey_name):
    """Register on SN6. Requires wallet password."""
    print(f"\n[5/7] Registering on SN{SUBNET['netuid']}...")
    print(f"  Estimated burn: ~{SUBNET['burn_estimate_tao']} TAO")

    check = run(f"python3 -c \"\nimport bittensor as bt, json, os\nsub = bt.Subtensor('finney')\nmg = sub.metagraph({SUBNET['netuid']})\nhk_path = os.path.expanduser('~/.bittensor/wallets/{wallet_name}/hotkeys/{hotkey_name}')\nss58 = json.load(open(hk_path))['ss58Address']\nif ss58 in mg.hotkeys:\n    print(f'REGISTERED UID {{mg.hotkeys.index(ss58)}}')\nelse:\n    print('NOT_REGISTERED')\nsub.substrate.close()\n\"", check=False)

    if check and "REGISTERED" in check and "NOT" not in check:
        print(f"  Already registered: {check}")
        return True

    print("  *** REQUIRES YOUR WALLET PASSWORD ***")
    print(f"  Run this command manually:")
    btcli = os.path.expanduser("~/synth-subnet/.venv/bin/btcli")
    print(f"  {btcli} subnet recycle_register --netuid {SUBNET['netuid']} --wallet.name {wallet_name} --wallet.hotkey {hotkey_name} --subtensor.network finney")
    return "MANUAL"


def configure(repo_path, wallet_name, hotkey_name):
    """Print agent upload instructions — no traditional config."""
    print("\n[6/7] Agent upload instructions...")
    print("")
    print("  SN6 Numinous uses a DIFFERENT pattern than other subnets:")
    print("  Instead of running a miner process, you UPLOAD a forecasting agent.")
    print("")
    print("  Steps to upload your agent:")
    print("  1. Write your forecasting agent (Python class)")
    print("  2. Use numi CLI to upload:")
    print(f"     numi upload --wallet.name {wallet_name} --wallet.hotkey {hotkey_name}")
    print("  3. Your agent runs on Numinous infrastructure — no PM2 needed")
    print("")
    print("  See Numinous documentation for agent API and examples.")
    print("  https://docs.numinous.ai/")
    return True


def start_miner(repo_path, wallet_name, hotkey_name):
    """No traditional miner to start — agent is uploaded."""
    print("\n[7/7] No PM2 process needed...")
    print("  SN6 agents run on Numinous infrastructure after upload.")
    print("  Use 'numi status' to check your agent's status.")
    print(f"  Or run: python3 {__file__} status")
    return True


def check_status():
    """Check on-chain status."""
    print(f"\n=== SN{SUBNET['netuid']} {SUBNET['name']} Status ===")

    # Check numi CLI status
    result = run("numi status", check=False)
    if result:
        print(f"  numi: {result}")

    # On-chain status
    run(f"python3 -c \"\nimport bittensor as bt\nsub = bt.Subtensor('finney')\nmg = sub.metagraph({SUBNET['netuid']})\nprint(f'Total UIDs: {{mg.n}}')\nprint(f'Active miners: {{sum(1 for i in range(mg.n) if float(mg.incentive[i]) > 0)}}')\nsub.substrate.close()\n\"", check=False)


def deploy(wallet_name, hotkey_name):
    """Full deployment pipeline."""
    print(f"{'='*60}")
    print(f"  SubnetAIQ Deploy Skill — SN{SUBNET['netuid']} {SUBNET['name']}")
    print(f"  Type: {SUBNET['type']}")
    print(f"  Difficulty: {SUBNET['difficulty']}")
    print(f"  Burn: ~{SUBNET['burn_estimate_tao']} TAO")
    print(f"  SPECIAL: Upload agent via numi CLI — no PM2 process")
    print(f"{'='*60}")

    home = os.path.expanduser("~")

    if not check_gpu():
        print("\nDEPLOY FAILED: Hardware requirements not met.")
        return False

    repo_path = clone_repo(home)
    if not repo_path:
        print("\nDEPLOY FAILED: Could not set up numinous.")
        return False

    if not setup_venv(repo_path):
        print("\nDEPLOY FAILED: Could not verify numinous.")
        return False

    if not create_hotkey(wallet_name, hotkey_name):
        print("\nDEPLOY FAILED: Could not create hotkey.")
        return False

    reg_result = register(wallet_name, hotkey_name)
    if reg_result == "MANUAL":
        print("\n!! PAUSED: Run the registration command above, then re-run this script.")
        return False
    if not reg_result:
        print("\nDEPLOY FAILED: Registration failed.")
        return False

    if not configure(repo_path, wallet_name, hotkey_name):
        print("\nDEPLOY FAILED: Configuration failed.")
        return False

    if not start_miner(repo_path, wallet_name, hotkey_name):
        print("\nDEPLOY WARNING: Issue during finalization.")
        return False

    print(f"\n{'='*60}")
    print(f"  SN{SUBNET['netuid']} {SUBNET['name']} — SETUP COMPLETE")
    print(f"  NEXT: Write and upload your forecasting agent via numi CLI")
    print(f"  Check: python3 {__file__} status")
    print(f"{'='*60}")
    return True


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description=f"SubnetAIQ Deploy Skill — SN{SUBNET['netuid']} {SUBNET['name']}")
    parser.add_argument("action", choices=["deploy", "status", "stop"], help="Action to perform")
    parser.add_argument("--wallet", default="finflow_miner", help="Wallet name")
    parser.add_argument("--hotkey", default=f"sn{SUBNET['netuid']}_01", help="Hotkey name")
    args = parser.parse_args()

    if args.action == "deploy":
        deploy(args.wallet, args.hotkey)
    elif args.action == "status":
        check_status()
    elif args.action == "stop":
        print("SN6 Numinous: No local process to stop.")
        print("Use numi CLI to manage your uploaded agent.")
