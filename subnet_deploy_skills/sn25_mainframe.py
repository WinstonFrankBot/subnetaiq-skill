"""SubnetAIQ Deploy Skill — SN25 Mainframe (Protein Folding)
Executable mining skill. Agent runs this → miner deployed → earning TAO.

What it does:
1. Checks hardware compatibility (8GB+ VRAM)
2. Clones repo + sets up venv
3. Creates hotkey
4. Registers on-chain
5. Configures environment (RQLite cluster + JOIN_ADDR)
6. Starts miner via PM2
7. Verifies earning status

WARNING: GJP server is currently DOWN. Miner may not earn until resolved.

Usage:
  python sn25_mainframe.py deploy --wallet finflow_miner --machine beast
  python sn25_mainframe.py status
  python sn25_mainframe.py stop
...
SUBNET = {
    "netuid": 25,
    "name": "Mainframe",
    "repo": "https://github.com/macrocosm-os/folding.git",
    "repo_dir": "folding",
...
def check_gpu():
    ...
    if not result:
        print("  No NVIDIA GPU found. SN25 needs GPU with 8GB+ VRAM.")
        return False
    ...
    print("")
    print("  !!! WARNING: GJP server is currently DOWN !!!")
    print("  !!! Miner may not earn emissions until GJP is restored !!!")
    return True
...
def register(wallet_name, hotkey_name):
    """Register on SN25. Requires wallet password."""
...
def configure(repo_path, wallet_name, hotkey_name):
    """Configure the miner environment with RQLite settings."""
    print("\n[6/7] Configuring miner...")
    env_file = os.path.join(repo_path, ".env")
    config = f"""# SN25 Mainframe (Protein Folding) — Auto-configured by SubnetAIQ Deploy Skill
...
DEVICE=cuda
# RQLite cluster settings — MUST configure JOIN_ADDR
JOIN_ADDR=YOUR_RQLITE_CLUSTER_ADDRESS_HERE
# Example: JOIN_ADDR=http://localhost:4001
# WARNING: GJP server currently DOWN — miner may not earn until restored
"""
    with open(env_file, "w") as f:
        f.write(config)
    print(f"  Config written: {env_file}")
    print(f"  !!! ACTION REQUIRED: Edit {env_file} and set JOIN_ADDR for your RQLite cluster !!!")
    print(f"  !!! WARNING: GJP server currently DOWN !!!")
    return True
...
def start_miner(repo_path, wallet_name, hotkey_name):
    ...
    miner_name = f"sn{SUBNET['netuid']}-mainframe"
...
def check_status():
    ...
    print("  WARNING: GJP server currently DOWN")
...
def deploy(wallet_name, hotkey_name):
    ...
    print(f"  WARNING: GJP server currently DOWN")
    print(f"  NOTE: Requires RQLite cluster (configure JOIN_ADDR)")
    print(f"{'='*60}")
    ...
    print(f"  Miner: sn{SUBNET['netuid']}-mainframe")
    print(f"  Port: {SUBNET['axon_port']}")
    print(f"  WARNING: GJP server DOWN — may not earn until restored")
...
if __name__ == "__main__":
    ...
    if args.action == "deploy":
        ...
        run(f"pm2 stop sn{SUBNET['netuid']}-mainframe", check=False)
        print("Miner stopped.")
