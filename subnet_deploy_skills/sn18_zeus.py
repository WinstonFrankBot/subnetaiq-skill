"""SubnetAIQ Deploy Skill — SN18 Zeus
...
Usage:
  python sn18_zeus.py deploy --wallet finflow_miner --machine beast
  python sn18_zeus.py status
  python sn18_zeus.py stop
...
SUBNET = {
    "netuid": 18,
    "name": "Zeus",
    "repo": "https://github.com/Orpheus-AI/Zeus.git",
    "repo_dir": "Zeus",
    "github": "Orpheus-AI/Zeus",
    "min_vram_gb": 8,
    "python_version": "3.10",
    "difficulty": "Expert",
...
def check_gpu():
    ...
    if not result:
        print("  No NVIDIA GPU found. SN18 Zeus needs GPU with 8GB+ VRAM.")
        return False
...
def register(wallet_name, hotkey_name):
    """Register on SN18. Requires wallet password."""
...
def configure(repo_path, wallet_name, hotkey_name):
    ...
    config = f"""# SN18 Zeus — Auto-configured by SubnetAIQ Deploy Skill
...
DEVICE=cuda
# Weather prediction model settings
# Requires weather data sources — see Zeus README for data setup
"""
...
def start_miner(repo_path, wallet_name, hotkey_name):
    ...
    miner_name = f"sn{SUBNET['netuid']}-zeus"
...
def deploy(wallet_name, hotkey_name):
    ...
    print(f"  Miner: sn{SUBNET['netuid']}-zeus")
...
if __name__ == "__main__":
    ...
    if args.action == "deploy":
        ...
        run(f"pm2 stop sn{SUBNET['netuid']}-zeus", check=False)
        print("Miner stopped.")
